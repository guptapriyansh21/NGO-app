...
public class UploadVideo extends AppCompatActivity {
    ...
    //    ArrayList<String> lstUploadedImage = new ArrayList<>();
        ArrayList<String> lstDisplayName = new ArrayList<>();
    String album_name;
...
//            lstUploadedImage.clear();
            if(album_name.length()>0){
                ...
                while (count < mArrayUri.size()) {
                    ...
                    StorageReference sRef = mStorageReference.child(Constant.STORAGE_PATH_UPLOADS).child(Constant.upload_video).child(album_name).child(album_name+"_"+System.currentTimeMillis());
                    sRef.putFile(uri)
...
//                                lstUploadedImage.add(taskSnapshot.getDownloadUrl().toString());
                                    ImageUrl upload = new ImageUrl(taskSnapshot.getDownloadUrl().toString(), Constant.getDate(), album_name, notice, (-1 * System.currentTimeMillis()),lstDisplayName.get(temp_count));
                                    myRef.child(Constant.upload_video).push().setValue(upload);
                                    Toast.makeText(context, "File "+(temp_count + 1) + " Uploaded Successfully", Toast.LENGTH_SHORT).show();
...
//                            displayName=displayName+"\n"+displayName;
                    lstDisplayName.add(displayName);
